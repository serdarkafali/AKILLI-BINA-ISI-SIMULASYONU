...
public class KonforOraniZamanGrafik extends Application {
    ...
    public void start(Stage stage) {
        ...
        List<Integer> greedy = konforZaman("GREEDY");
        List<Integer> dp = konforZaman("DP");
        List<Integer> normal = konforZaman("NORMAL");

        CategoryAxis x = new CategoryAxis();
        x.setLabel("Zaman (dk)");

        NumberAxis y = new NumberAxis(0, 100, 10);
        y.setLabel("Konfor Oranı (%)");

        LineChart<String, Number> chart = new LineChart<>(x, y);
        chart.setTitle("Zamana Göre Konfor Oranı");
        ...
        stage.setTitle("Konfor Oranı - Zaman");
    ...
    private List<Integer> konforZaman(String tip) {

        List<Oda> odalar = ornekOdaListesi();
        List<Integer> oranlar = new ArrayList<>();
        ...
        for (int dakika = 1; dakika <= MAX_SURE; dakika++) {
            ...
            for (Oda o : odalar) {
                if (o.simdikiSicaklik >= o.comfortMin
                        && o.simdikiSicaklik <= o.comfortMax) {
                    konforlu++;
                }
            }

            int oran = (int) Math.round(
                    (konforlu * 100.0) / odalar.size()
            );
            oranlar.add(oran);
        }
